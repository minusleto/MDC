...
В `type` указывается тот же ID здания, что и в таблицах стоимости в начале страницы (`industrial_complex`, `arms_factory`, `dockyard` и т.д.) — отдельно повторять список незачем, id везде одинаковые.
