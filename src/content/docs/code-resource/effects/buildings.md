...
| Постройка | ID | Стоимость |
|---|---|---:|
| Синтетический НПЗ | `synthetic_refinery` | $4.25 |
| Резиновый завод* | `rubber_refinery` | $1.75 (конкретный регион) / $4.25 (случайный регион) |
...
\* Стоимость реально отличается в 2.4 раза в зависимости от того, строите вы в случайном или в конкретном регионе — похоже на баг в самом моде, но задокументировано как есть.
...
:::caution
Стоимость через `_random_` и `_state_` варианты для этого здания различается в 2.4 раза (см. таблицу стоимости выше) — похоже на баг в самом моде.
:::
