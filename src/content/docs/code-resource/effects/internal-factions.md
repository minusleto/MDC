...
- **Мнение само плавно ползёт к минимуму каждый месяц** (обычно к тем же 50, если не сдвинуто уникальной идеей страны) — то есть если ничего не трогать, оно со временем осядет обратно.
...
| Страновые | `iranian_quds_force` (Иран) | `change_iranian_quds_force_opinion` | Дрейф коммунизма, влияние на наступление |
...
Мнение фракций напрямую участвует в системе выборов MD: каждая активная фракция добавляет от **-2 до +2** к переменной `campaign_funding_count` в зависимости от того, насколько она враждебна/благосклонна (hostile/negative/indifferent/positive/enthusiastic). Это считается автоматически эффектом `display_election_campaign_status`, вручную дёргать не нужно.

:::note
В старом списке команд встречались `change_isi_pakistan_opinion`, `change_vevak_opinion`, `change_the_bazaar_opinion`, `change_irgc_opinion` — в основном файле системы (`00_internal_faction_effects.txt`) их нет, это либо страновые надстройки в других файлах, либо устаревшие/переименованные названия. Пока не подтверждены — не включены в таблицу выше.
:::
