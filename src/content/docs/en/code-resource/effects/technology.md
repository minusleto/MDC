...
If both `bonus` and `ahead_reduction` are specified, they belong to the same limited-use research bonus:
...
The single use can be spent either for **+50% research speed** or for **reducing the ahead-of-time penalty by 1 year**. These are not permanent country modifiers.
...
Here the single use provides either **+100% research speed** or **a 1-year reduction to the ahead-of-time penalty** for an applicable technology.
...
The categories below are grouped by practical purpose, starting with the most basic and commonly used ones. The list is based on `technology_Categories` from the source file; categories repeated in the source are shown only once.
...
| `CAT_armor` — all armor technologies |
...
## Economy, industry and civilian technology
...
## Computing, internet and special civilian technology
...
## Land equipment: artillery, anti-air and anti-tank
...
| `CAT_aa` — all anti-air |
| `CAT_l_aa` — light/portable anti-air |
...
| `CAT_drones` — drones |
| `CAT_wings` — wing forms/configurations |
...
| `CAT_air_camera` — targeting pods and cameras |
...
| `CAT_vls_air_systems` — VLS for air systems |
| `CAT_vls_land_systems` — VLS for land systems |
